...
It creates a pi-tui `Input`-backed submenu component with enter-to-confirm / escape-to-cancel handling. Both implementations are identical — they import the same `Input`, `Key`, `matchesKey` from `@mariozechner/pi-tui`, create an `Input` instance, set its value, and return `{ render, invalidate, handleInput }`.
...
3. Ensure `@mariozechner/pi-tui` is already a peerDependency of `supi-core` (it should be, since `settings-ui.ts` imports from it).
...
Then verify:
- `supi-core` already has `@mariozechner/pi-tui` in dependencies/peerDependencies
