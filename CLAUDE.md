...
- Child-only adapters such as `supi-agents` and `supi-review` may bundle Code Intelligence solely to pass its headless factory into owned child sessions; they must not load the full parent extension, and `scripts/__tests__/bundled-extension-refs.test.mjs` documents this exception.
