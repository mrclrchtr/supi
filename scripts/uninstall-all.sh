...
if [[ "${1:-}" == "-l" ]]; then
  ...
  echo "Usage: uninstall-all.sh [-l]"
  echo ""
  echo "  (no args)  Uninstall the full SuPi stack globally (release + beta)"
  echo "  -l         Uninstall the full SuPi stack project-locally (.pi/settings.json)"
  exit 1
fi
...
echo "Uninstalling the full SuPi stack ($SCOPE)…"
echo ""
