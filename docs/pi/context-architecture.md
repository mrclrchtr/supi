...
- **Skill discipline** — catalog entries are Tier 1; full `tool-rendering-review.md` bodies are lazy-loaded by the agent on demand. Ship short, specific descriptions; `disable-model-invocation: true` hides a skill from the catalog and forces `/skill:name`.
