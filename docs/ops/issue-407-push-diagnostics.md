...
- Refreshes use shared freshness checks and return exact `requested`, `confirmed`, `unconfirmed`, `failed`, and `removed` coverage. They retain unchanged documents, resynchronize changed or invalidated documents, and do not reopen a document to obtain diagnostic confirmation. Diagnostic timing records the request source; publication counts remain observation telemetry.
