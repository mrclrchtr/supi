...
- The target workflow may refine a workspace-symbol hit through document symbols, then use a structural identifier snap when provider-backed syntax can establish the name.
- Single-target and disambiguation paths apply the same refinement policy.
...
- Target-handle identity hashes stable fields (`cwd`, file, name, kind, container, fingerprint), not display position. This preserves handle reuse when refinement changes only the anchor position.
