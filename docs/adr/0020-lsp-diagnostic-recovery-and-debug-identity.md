...
On push-only routes, an open document still unconfirmed after the settle window is closed and reopened over the protocol (`didClose` + `didOpen` with current content), because a clean file receives no push on `didChange` at all — `typescript-language-server` skips empty-to-empty updates but publishes on `didOpen`. The reopen keeps the diagnostic cache entry and version history intact and does not discard other documents' server state; evidence then settles again within a bounded second window (`min(maxWaitMs, quietMs × 4)` for `refresh-open`, a fixed 1 s wait for `sync-file`).
