...
Provide a cross-extension data sharing registry in supi-core that allows extensions to register named data providers. These providers expose structured data (e.g., rewrite statistics from RTK, cache health from cache-monitor) that can be rendered in the `/supi-context` command and other analysis views.
