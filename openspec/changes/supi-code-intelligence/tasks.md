...
- [ ] 2.3 Add `supi-code-intelligence` to the root `package.json` `pi.extensions` array after `supi-lsp` and `supi-tree-sitter`
- [ ] 2.4 Add the package to `packages/supi/package.json`, create a wrapper entrypoint such as `packages/supi/code-intelligence.ts`, and register it in the meta-package `pi.extensions` list after the lower-layer wrappers
...
- [ ] 3.2 Detect notable public/exported API surfaces and config/settings entrypoints cheaply enough for briefs and overview summaries
- [ ] 3.3 Implement module-boundary and focus-path resolution for project, module/directory, and single-file analysis
- [ ] 3.4 Implement dependency graph extraction that distinguishes internal module edges, reverse dependencies/dependents, and external package dependencies where available
- [ ] 3.5 Implement structural enrichment using `createTreeSitterSession()` for outlines, imports, exports, and node context when supported
- [ ] 3.6 Implement semantic enrichment using `getSessionLspService(ctx.cwd)` for document symbols, workspace symbols, references, implementations, and diagnostics when available
- [ ] 3.7 Implement a shared architecture model that can power both the auto-injected overview and on-demand `brief` output
- [ ] 3.8 Handle empty or source-only projects without producing noisy empty architecture sections
- [ ] 3.9 Dispose or reuse Tree-sitter sessions deliberately so repeated briefs do not leak parser resources
