...
If semantic reference data is unavailable, the system SHALL either return clearly labeled `structural` or `heuristic` impact output, or explain that impact analysis is `unavailable`. When semantic enrichment is still pending but bounded structural or heuristic evidence is already available, the tool SHOULD return the prompt partial result rather than block for deeper analysis and SHOULD suggest a retry only when a later rerun is likely to materially improve confidence.
...
- **WHEN** the agent requests `action: "affected"` while semantic reference data is still pending but usable structural evidence already exists
- **THEN** the tool returns the bounded structural result promptly instead of waiting indefinitely for semantic enrichment
