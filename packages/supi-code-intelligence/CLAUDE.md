...
- **`code_find`**: AST kinds are exactly `definition`, `import`, `export`, `call`, `type`, `interface`, `class`, `method`, and `enum`; test identity is not inferred. AST `call` matches by written name. Modes never silently fall back. Scope is always an explicit non-empty string array.
