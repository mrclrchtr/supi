...
export const CODE_INTELLIGENCE_TOOL_SPECS = [
  ...
  },
  {
    ...
    label: "Code Find",
    description:
      'Unified ranked code search with strict mode dispatch: default/`mode: "text"` is literal ripgrep, `mode: "regex"` is ripgrep regex, `mode: "semantic"` is LSP workspace-symbol search, and `mode: "ast"` is tree-sitter structured search. `mode: "ast"` requires `kind` and currently supports `definition`, `import`, `export`, `call`, `type`, and `interface`. `mode: "text"`, `mode: "regex"`, and `mode: "semantic"` do not accept `kind`. `code_find` with `mode: "semantic"` does not silently fall back to text search, and unsupported mode/kind combinations fail. AST `call` mode matches call-site identifiers by name (text), not by symbol identity — it finds call sites of any function with that name, including shadowed definitions. Use `code_graph` with the `"references"` relation for identity-aware callers when you have a resolved target.',
    ...
    basePromptGuidelines: [
      ...
      'code_find with `mode: "ast"` requires `kind` and supports `definition`, `import`, `export`, `call`, `type`, and `interface`. AST `call` mode matches identifiers by name only — it cannot distinguish different symbols that share a name. Prefer `code_graph` with the `"references"` relation to find identity-aware callers for a specific resolved symbol.',
      ...
      'When using `mode: "ast"` with `kind: "call"`, be aware: AST call search matches call-site identifiers by name only — it returns call sites for every symbol with that name, not just one definition. Use `code_graph` with `relations: ["references"]` on a resolved target for symbol-identity-aware caller discovery.',
    ],
