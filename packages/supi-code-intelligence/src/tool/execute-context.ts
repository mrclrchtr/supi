...
import { runOrientationMode } from "./orientation/orientation-mode.ts";
import { resolvePreciseTarget } from "./orientation/precise-target.ts";
...
export async function executeOrientationTool(
  ...
  if (hasTargetId || hasCoords) {
    const preciseResult = await resolvePreciseTarget(params, ctx, hasCoords);
