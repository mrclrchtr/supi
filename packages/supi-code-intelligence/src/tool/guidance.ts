...
export const CODE_INTELLIGENCE_TOOL_PROMPT_SURFACES: CodeIntelligenceToolPromptSurfaceMap = {
  code_resolve: {
    description:
      "Resolve references to evidence-backed code targets and stable targetId/spanId handles for follow-up code_* tools. Accepts query/symbol/file or anchored file+line+character; anchored resolution snaps only to real provider-backed symbols, and whitespace/comment/non-symbol points fail with code_inspect guidance. No text-search fallback; ambiguous results return ranked candidates. Output is truncated to 2000 lines / 50KB.",
    promptSnippet: "code_resolve — resolve references into precise targets and target handles",
    promptGuidelines: [
      "Use code_resolve before graph/impact/refactor/orientation when the target is ambiguous or symbol-only.",
      "When code_resolve is ambiguous, retry with a candidate file + line + character.",
    ],
  },
  code_inspect: {
    description:
      "Inspect one exact file position for syntax, symbol, hover/definition, nearby diagnostics, and code-action titles. Requires file, line, and character; use for point facts, not broad orientation. Unavailable providers are disclosed; code actions are advisory only. Output is truncated to 2000 lines / 50KB.",
    promptSnippet: "code_inspect — factual point inspection",
    promptGuidelines: [
      "Use code_inspect for exact point facts; use code_orientation for broader orientation or code_resolve for reusable targets.",
    ],
  },
  code_orientation: {
    description:
    ...
    promptGuidelines: [
      ...
      "Use code_resolve first for bare or ambiguous symbols, then pass targetId to code_orientation.",
    ],
  },
  code_graph: {
    description:
    ...
    promptGuidelines: [
      "Use code_graph for references, callees, implementations, imports/exports, or tests; prefer targetId from code_resolve.",
      'In code_graph, choose relations explicitly when not using default references; use calleeDepth:"deep" only when nested calls matter.',
    ],
  },
  code_impact: {
    description:
      "Estimate blast radius for a resolved target or user-supplied changeSetFiles. changeSetFiles are explicit files, not inferred from git and no line-level diff; change-only requests report insufficient evidence. Uses semantic references plus deterministic test discovery when available; no heuristic search fallback. Output includes read-next guidance and is truncated to 2000 lines / 50KB.",
    promptSnippet: "code_impact — blast radius and impact",
    promptGuidelines: [
      "Use code_impact before edits when you need blast radius, affected files, or likely tests.",
      "Use code_graph instead of code_impact when you only need a plain reference list.",
    ],
  },
  code_find: {
    description:
      'Search code with explicit modes: text literal (default), regex, semantic workspace-symbol, or ast structured. mode:"ast" requires `kind` (definition/import/export/call/type/interface/class/method/enum/test); text/regex/semantic reject kind. Semantic mode uses LSP only and does not silently fall back. AST call matches written call-site names, not by symbol identity; use code_graph references on a resolved target for symbol-identity-aware callers. Output is truncated to 2000 lines / 50KB.',
    ...
    promptGuidelines: [
      ...
      'For code_find, pass kind only with mode:"ast"; mode:"semantic" never falls back to text search.',
    ],
  },
  code_refactor_plan: {
    description:
      "Preview a semantic refactor plan and return planId; never mutates files. Supports rename_symbol (rename alias) and extract_function/extract_variable when LSP returns precise edits. Requires targetId or anchored file+line+character, plus operation-specific newName/range. Apply separately with code_refactor_apply. Output is truncated to 2000 lines / 50KB.",
  ...
  code_refactor_apply: {
    description:
      "Apply a stored refactor plan by planId. Revalidates plan freshness and file fingerprints before mutating; does not compose new plans. Output is truncated to 2000 lines / 50KB.",
  ...
  code_health: {
    description:
      "Report workspace health: diagnostics, language-server status, dirty files, coverage, and unused code. Use scope/include/level to narrow; refresh:true recovers stale diagnostics. Missing coverage/unused reports are disclosed. Output is truncated to 2000 lines / 50KB.",
    promptSnippet:
