...
import { findLikelyTests } from "../../../../src/tool/impact/orchestrate.ts";
...
describe("code_impact tool", () => {
  ...
  it("reports likely tests for target-based impact with zero semantic references (regression for audit failure)", async () => {
    // Package layout: source in src/tool/graph/execute.ts
    // Test in __tests__/unit/execute-graph.test.ts
    // No semantic reference from test to source.
    mkdirSync(path.join(tmpDir, "src", "tool", "graph"), { recursive: true });
    writeFileSync(
    ...
    writeFileSync(
      path.join(tmpDir, "__tests__/unit/execute-graph.test.ts"),
    ...
    const resolveResult = (await resolveTool.execute(
      "impact-resolve",
      { file: "src/tool/graph/execute.ts", line: 1, character: 17 },
      undefined,
      undefined,
      makeCtx({ cwd: tmpDir }),
    )) as {
    ...
    expect(result.content[0].text).toContain("__tests__/unit/execute-graph.test.ts");
    expect(result.details?.type).toBe("impact");
    if (result.details?.type === "impact") {
  ...
  it("surfaces semantic provenance and tests details for target-based semantic companion tests", async () => {
    mkdirSync(path.join(tmpDir, "src", "tool", "find"), { recursive: true });
    writeFileSync(
    ...
    const resolveResult = (await resolveTool.execute(
      ...
      { file: "src/tool/find/execute.ts", line: 1, character: 20 },
      undefined,
      undefined,
      makeCtx({ cwd: tmpDir }),
    )) as {
  ...
  it("discovers bounded tool test via conventions-only impact", async () => {
    // Source src/tool/find/execute.ts, test __tests__/unit/code-find-tool.test.ts
    mkdirSync(path.join(tmpDir, "src", "tool", "find"), { recursive: true });
    writeFileSync(
      path.join(tmpDir, "src/tool/find/execute.ts"),
      "export function executeFind() { return 1; }\n",
    ...
    const resolveResult = (await resolveTool.execute(
      "impact-resolve",
      { file: "src/tool/find/execute.ts", line: 1, character: 17 },
      undefined,
      undefined,
      makeCtx({ cwd: tmpDir }),
    )) as {
  ...
  it("ignores __tests__/helpers support files for target-based likely tests", async () => {
    ...
    registerMockProvider(tmpDir, {
      ...
      references: async (file) =>
        file.endsWith("execute-graph.ts")
          ? [
              {
                uri: `file://${path.join(tmpDir, "__tests__/unit/execute-graph.test.ts")}`,
                range: {
                  start: { line: 0, character: 9 },
    ...
    if (result.details?.type === "impact") {
      expect(result.details.data?.likelyTests).toContain("__tests__/unit/execute-graph.test.ts");
      ...
      expect(result.details.data?.likelyTestCommands).toContain(
        "pnpm vitest run __tests__/unit/execute-graph.test.ts --reporter=verbose",
      );
  ...
  it("reports likely tests for change-set impact with package-layout mirrors", async () => {
    // Package layout: source src/tool/graph/execute.ts, test __tests__/unit/execute-graph.test.ts
    mkdirSync(path.join(tmpDir, "src", "tool", "graph"), { recursive: true });
    writeFileSync(
      path.join(tmpDir, "src/tool/graph/execute.ts"),
      "export function executeGraph() { return 1; }\n",
    ...
    const result = (await impactTool.execute(
      "impact-changed-pkg",
      {
        changeSetFiles: ["src/tool/graph/execute.ts"],
        includeTests: true,
      },
      undefined,
      undefined,
  ...
  it("ignores semantic-only test references for change-set impact", async () => {
    ...
    const result = (await impactTool.execute(
      ...
      {
        changeSetFiles: ["src/tool/find/execute.ts"],
        includeTests: true,
      },
      undefined,
      undefined,
...
describe("findLikelyTests boundary awareness", () => {
  ...
  it("does not match tool-specs.ts as a test file (regression for substring bug)", () => {
    const affected = pathSet("packages/supi-code-intelligence/src/tool/specs.ts");
    const tests = findLikelyTests(affected, tmpDir);
    expect(tests.map((t) => t.path)).not.toContain(
      path.resolve(tmpDir, "packages/supi-code-intelligence/src/tool/specs.ts"),
    );
  });
