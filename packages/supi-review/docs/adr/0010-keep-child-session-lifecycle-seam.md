...
`supi-review` retains `child-session-runner.ts` as a thin adapter: it supplies review-owned resources and structured completion, maps runtime outcomes to the legacy review vocabulary, and attaches audit/capability observers. Lifecycle contract tests now live at the runtime's public `startAgentRun()` seam, while review tests cover adapter and behavior parity. Do not restore the deleted review-owned lifecycle machinery; extend the runtime API or its contract tests when generic lifecycle behavior changes.
