...
export function buildPlannerSystemPrompt(): string {
  return [
    ...
    `Reviewers receive read, bash, grep, ${HEADLESS_INSPECTION_TOOL_NAMES.join(", ")}, and ${REVIEW_CHILD_TOOL_SPECS.submitReview.name}.`,
    ...
    `Submit one valid draft with ${REVIEW_CHILD_TOOL_SPECS.submitPlannerDraft.name}; if the tool rejects it, correct the draft and retry.`,
  ].join("\n");
}
