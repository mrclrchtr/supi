...
export const MAX_CHILD_ERROR_CHARACTERS = 500;
...
/** Redact, normalize, and bound provider-owned error text before session persistence. */
export function sanitizeChildErrorText(value: unknown): string | undefined {
  if (typeof value !== "string" || !value.trim()) return undefined;
  const redacted = redactDebugData(value)
  ...
  if (normalized.length <= MAX_CHILD_ERROR_CHARACTERS) return normalized;
  let end = MAX_CHILD_ERROR_CHARACTERS - 1;
