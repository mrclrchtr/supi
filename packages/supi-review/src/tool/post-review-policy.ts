...
export function buildPostReviewInstruction(
  ...
  if (output.nextOffset !== undefined) {
    lines.push(
      `Before responding, retrieve every remaining output page with supi_review_output, starting with ${JSON.stringify({ artifactId: output.artifactId, offset: output.nextOffset })}.`,
    );
  }
