...
An explicit diagnostic refresh also retries failed startup and exhausted process-crash routes in scope. It returns separate bounded startup-retry and process-crash reports. The process report gives exact recovered, skipped, failed, and exhausted route counts. The startup report gives exact recovered and failed route counts. Each report has up to 16 route entries and an omitted-entry count. Each entry names the server and workspace-relative root. A skipped process route recommends an exact-file refresh; a failed or exhausted route recommends another explicit refresh. A failed entry may include only the caught error message, limited to 512 characters. Broad refreshes use bounded discovery and retry routes whose roots overlap the requested scope, without requiring retained files. Exact-file readiness uses only the file's route. Each route gets at most one startup or replacement attempt per refresh call; a later refresh can try again. Healthy routes are reused, not restarted by this retry path. Recovery does not consume an attempt for a skip or claim process readiness as diagnostic evidence.
