...
`ProjectServerInfo.statusReason` is present only for process-crash recovery states: `process-crashed`, `process-crash-recovery-pending`, or `process-crash-recovery-exhausted`. A route stays in `error` status for all three states. The next file-routed semantic or explicit diagnostic operation can consume one shared replacement attempt; workspace-wide operations and passive status reads do not start it.
