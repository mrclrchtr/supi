...
Semantic and structural provider methods also accept optional `CodeRequestControl` metadata. It contains a caller `AbortSignal` and an absolute Unix-epoch deadline. Adapters preserve the same object. Canonical helpers identify cancellation and deadline expiry across bundled package copies. Cooperative structural substrates apply the control; unsupported substrates can preserve it without applying behavior.
