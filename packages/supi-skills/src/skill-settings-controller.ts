...
import type { SessionCapabilitySkill } from "@mrclrchtr/supi-core/session";
import type {
  ScopedFieldValue,
  SettingsAction,
  SettingsApplyResult,
...
export class SkillSettingsController {
  ...
  /** Return PI's loaded skill winners that persistent settings allow in the prompt. */
  eligibleModelSkills(ctx: ExtensionCommandContext): SessionCapabilitySkill[] {
    const records = this.records("project", ctx);
    return (ctx.getSystemPromptOptions().skills ?? [])
      .filter((skill) => {
        const record = records.get(skill.name);
        return !resolveInvocation({
          name: skill.name,
          sourceDefault: record ? sourceDefault(record) : (skill.disableModelInvocation ?? false),
          scope: "project",
          cwd: this.cwd,
          projectTrusted: this.projectTrusted,
          homeDir: this.homeDir,
