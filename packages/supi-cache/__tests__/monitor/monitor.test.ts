...
describe("/supi-cache-history command", () => {
  ...
  it("sends a custom message with turn snapshot", async () => {
    ...
    const cmd = pi.commands.get("supi-cache-history") as { handler: (args: string, ctx: unknown) => Promise<unknown> };
...
describe("/supi-cache-forensics command", () => {
  ...
  it("calls runForensics and sends a message with the result", async () => {
    ...
    const cmd = pi.commands.get("supi-cache-forensics") as { handler: (args: string, ctx: unknown) => Promise<unknown> };
