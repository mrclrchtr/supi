...
describe("computeToolCallShape", () => {
  it.concurrent("captures string param shape with length and multiline", () => {
...
describe("stripHumanDetail", () => {
  it.concurrent("removes _pathsInvolved and _commandSummaries from findings", () => {
  ...
  it.concurrent("does not mutate the original findings", () => {
    const findings = [makeFinding()];
  ...
  it.concurrent("returns a new array, not the same reference", () => {
    const findings = [makeFinding()];
    const stripped = stripHumanDetail(findings);
    expect(stripped).not.toBe(findings);
  });

  it.concurrent("handles findings without human-only fields", () => {
  ...
  it.concurrent("handles empty array", () => {
