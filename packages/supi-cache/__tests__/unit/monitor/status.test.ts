...
describe("formatCacheStatus", () => {
  ...
  it("shows ↓ when hit rate decreased", () => {
    ...
    expect(formatCacheStatus(state)).toBe("TCH50%↓");
  });
  ...
  it("returns undefined when cacheSupported is false", () => {
    const state = new CacheMonitorState();
  ...
  it("returns undefined when current turn has undefined hitRate", () => {
    const state = new CacheMonitorState();
    state.recordTurn({ cacheRead: 8000, cacheWrite: 0, input: 2000 }, 1000);
  ...
  it("returns undefined when switching to a non-cache provider after cache-capable turn", () => {
    const state = new CacheMonitorState();
    ...
    // Should return undefined not 0%, because this turn has no cache data
