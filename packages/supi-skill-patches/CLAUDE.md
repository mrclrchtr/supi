...
- `patches/mattpocock-skills/files/` is the source of truth for SuPi changes. Keep one patch fragment per changed upstream file.
...
- Root `skills/<name>/` directories are generated outputs for skills.sh. Run `pnpm skills:sync` after the patched dependency changes.
...
Keep changes narrow. Preserve upstream files that do not need SuPi adaptation. Each generated skill must include `LICENSE.mattpocock`.
