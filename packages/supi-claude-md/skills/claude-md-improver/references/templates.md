...
> ⚠️ **When SuPi is active:** The `## Packages` table in this template is auto-generated by `code_intel brief` on the first turn. Consider omitting it or replacing with a `## Cross-Package Patterns` section that captures conventions not visible in manifests.
...
- `## Architecture` with directory tree — auto-discovered from file structure
